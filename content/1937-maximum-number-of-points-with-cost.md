...
class Solution:
    def maxPoints(self, points: List[List[int]]) -> int:
        ...
        dp[0] = points[0][:] # init the first row

        for i in range(1, rows):
            prev = (-inf, 0)

            def f(j):
                return prev[0] - abs(j - prev[1])

            for j in range(cols):
                # init from prev col, or based on prev
                dp[i][j] = max(dp[i - 1][j] + points[i][j], f(j) + points[i][j])
            ...
            for j in range(cols - 1, -1, -1):
                # update if prev is greater
                dp[i][j] = max(dp[i][j], f(j) + points[i][j])
