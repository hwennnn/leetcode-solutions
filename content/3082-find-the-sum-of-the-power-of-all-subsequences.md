...
class Solution:
    def sumOfPower(self, nums: List[int], k: int) -> int:
        ...
        @cache
        def go(index, ssum, count):
            if ssum == k: return pow(2, N - count, MOD)

            if ssum > k or index >= N: return 0

            skip = go(index + 1, ssum, count)
            take = go(index + 1, ssum + nums[index], count + 1)
