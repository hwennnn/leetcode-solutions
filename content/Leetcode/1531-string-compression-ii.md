...
class Solution:
    def getLengthOfOptimalCompression(self, s: str, k: int) -> int:
        ...
        @cache
        def go(index, prev, count, k):
            if k < 0: return inf

            if index == N: return 0

            if s[index] == prev:
                return min(go(index + 1, prev, count + 1, k) + int(count in (1, 9, 99)), go(index + 1, prev, count, k - 1))

            return min(1 + go(index + 1, s[index], 1, k), go(index + 1, prev, count, k - 1))
