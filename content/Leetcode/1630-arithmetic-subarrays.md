    ...
    def checkArithmeticSubarrays(self, nums: List[int], l: List[int], r: List[int]) -> List[bool]:
        ...
        def isArithmetic(start, end):
            ...
            for index in range(start, end + 1):
                mmin = min(mmin, nums[index])
                mmax = max(mmax, nums[index])
                s.add(nums[index])
            
            if (mmax - mmin) % (end - start) != 0: return False
            interval = (mmax - mmin) // (end - start)

            for i in range(1, end - start):
                if mmin + i * interval not in s: return False
