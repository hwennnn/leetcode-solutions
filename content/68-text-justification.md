...
class Solution:
    def fullJustify(self, words: List[str], maxWidth: int) -> List[str]:
        ...
        def helper(count, curr):
            if len(curr) == 1:
                return curr[0] + " " * (maxWidth - len(curr[0]))

            spaces, extra = divmod(maxWidth - count, len(curr) - 1)
            word = ""

            for i, x in enumerate(curr):
                word += x
                if i == len(curr) - 1: break
        ...
        for x in words:
            if count + len(curr) + len(x) > maxWidth:
                res.append(helper(count, curr))
                count = len(x)
                curr = [x]
            else:
                count += len(x)
