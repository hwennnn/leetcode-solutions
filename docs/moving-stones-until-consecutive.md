---
id: moving-stones-until-consecutive
title: Moving Stones Until Consecutive
description: Problem Description and Solution for Moving Stones Until Consecutive
sidebar_label: 1033. Moving Stones Until Consecutive
sidebar_position: 1033
---

# [1033. Moving Stones Until Consecutive](https://leetcode.com/problems/moving-stones-until-consecutive/)

```py title=moving-stones-until-consecutive.py
class Solution:
    def numMovesStones(self, a: int, b: int, c: int) -> List[int]:
        A = sorted([a, b, c])
        a, b, c = A
        if c - a == 2: return [0, 0]
        return [1 if min(c - b, b - a) <= 2 else 2, c - a - 2]
```


